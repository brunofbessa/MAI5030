...
import sga_rosenbrock
import cga_rosenbrock
import numpy as np
...
def main():
    ...
    start = timer()
    exec_sga = calcular_lista_geracoes_ga(sga_rosenbrock.main)
    fitness_sga = exec_sga[0]
    mat_cov_sga_g = exec_sga[1]
    end = timer()
    tempo_sga = end - start
    start = timer()
    exec_cga = calcular_lista_geracoes_ga(cga_rosenbrock.main)
