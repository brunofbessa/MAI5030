...
def main():
    ...
    pop_sga = pop_ga(sga_one_max.main)
    pop_cga = pop_ga(cga_one_max.main)
    compara_dist_mannwhitneyu(pop_sga, pop_cga)

    fig, ax = plt.subplots(1, 2)

    start = timer()
    fitness_sga = calcular_lista_geracoes_ga(sga_one_max.main)
    end = timer()
    tempo_sga = end - start

    start = timer()
    fitness_cga = calcular_lista_geracoes_ga(cga_one_max.main)
    end = timer()
    tempo_cga = end - start
    ...
    start = timer()
    fitness_sga = calcular_lista_populacoes_ga(sga_one_max.main)
    end = timer()
    tempo_sga = end - start
    start = timer()
    fitness_cga = calcular_lista_populacoes_ga(cga_one_max.main)
    end = timer()
    tempo_cga = end - start
